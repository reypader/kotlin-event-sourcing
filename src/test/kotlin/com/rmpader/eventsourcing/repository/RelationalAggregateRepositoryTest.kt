...
class RelationalAggregateRepositoryTest {
    ...
    fun setup() {
        ...
        repository = RelationalAggregateRepository(
    ...
    @Test
    fun `storeEvent should persist event to journal and outbox transactionally`() = runTest {
        // Given some event to be persisted
        val entityId = "test-entity-1"
        val event = TestEvent("test-event", 42)
        val eventRecord = AggregateRepository.EventRecord(
            entityId = entityId,
            event = event,
            sequenceNumber = 1L,
    ...
    @Test
    fun `storeEvent should rollback transaction on duplicate persist`() = runTest {
        // Given
        val entityId = "test-entity-duplicate"
        repository.storeEvent(
            AggregateRepository.EventRecord(
                entityId, TestEvent("event-1", 1), 1L, OffsetDateTime.now(ZoneOffset.UTC)
            )
        )
        TestDatabase.assertEventJournalCount(1L)
        TestDatabase.assertEventOutboxCount(1L)
        ...
        try {
            repository.storeEvent(
                AggregateRepository.EventRecord(
                    entityId, TestEvent("event-duplicate", 2), 1L, OffsetDateTime.now(ZoneOffset.UTC)
                )
            )
    ...
    @Test
    fun `storeEvent should rollback when conflicting with an orphan journal`() = runTest {
        ...
        try {
            repository.storeEvent(
                AggregateRepository.EventRecord(
                    entityId, TestEvent("new-event", 1), 1L, OffsetDateTime.now(ZoneOffset.UTC)
                )
            )
    ...
    @Test
    fun `storeEvent should rollback when conflicting with an orphan outbox`() = runTest {
    ...
    @Test
    fun `loadEvents should return empty flow when no events exist`() = runTest {
    ...
    @Test
    fun `loadEvents should return events ordered by sequence number`() = runTest {
    ...
    @Test
    fun `storeEvent should persist multiple events for same entity`() = runTest {
        // Given
        ...
        repository.storeEvent(
            AggregateRepository.EventRecord(
                entityId, TestEvent("event-2", 2), 2L, OffsetDateTime.now(ZoneOffset.UTC)
            )
        )
        repository.storeEvent(
            AggregateRepository.EventRecord(
                entityId, TestEvent("event-3", 3), 3L, OffsetDateTime.now(ZoneOffset.UTC)
            )
        )
    ...
    @Test
    fun `loadLatestSnapshot should return null when no snapshot exists`() = runTest {
        // Given
        val entityId = "non-existent"
